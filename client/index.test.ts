...
test("counter setup", async () => {
    ...
    const txHash = await connection.sendTransaction(tx, [adminKeyPair, counterKeyPair]);
    await connection.confirmTransaction(txHash);

    const counterAccount = await connection.getAccountInfo(counterKeyPair.publicKey);
    if (!counterAccount) {
